...
namespace CommandLineTools.CommandLine
{
    public interface IValidationMessages
    {
        ...
        string CommandAcceptsOnlyOneArgument(string command,
                                             int    argumentCount);

        string FileDoesNotExist(string filePath);

        string CommandAcceptsOnlyOneSubcommand(string command,
                                               string subcommandsSpecified);

        string OptionAcceptsOnlyOneArgument(string option,
                                            int    argumentCount);
        ...
        string UnrecognizedArgument(string   unrecognizedArg,
                                    string[] allowedValues);

        string UnrecognizedCommandOrArgument(string arg);

        string UnrecognizedOption(string   unrecognizedOption,
                                  string[] allowedValues);
    }
}